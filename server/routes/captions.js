...
async function runPythonTranscription(wavPath) {
  ...
  throw lastError || new Error("No working Python runtime found for caption transcription");
}
...
function secondsToVttTime(seconds) {
  ...
  return `${String(hrs).padStart(2, "0")}:${String(mins).padStart(2, "0")}:${secs
    .toFixed(3)
    .padStart(6, "0")}`;
}
...
async function ensureCaptionFiles(videoPath) {
  ...
  if (fs.existsSync(vttPath) && fs.existsSync(jsonPath)) {
    return {
      vttPath,
      jsonPath,
      segments: readExistingSegments(jsonPath),
    };
  }
  ...
  if (!segments.length) {
    throw new Error("No caption segments were produced from transcription");
  }
...
function splitSegmentsIntoPseudoWords(segments = []) {
  ...
  segments.forEach((segment) => {
    ...
    parts.forEach((word, index) => {
      ...
      const end =
        index === parts.length - 1 ? segEnd : segStart + slice * (index + 1);
...
async function streamPseudoRealtime(ws, segments = []) {
  ...
  for (const item of words) {
    ...
    const dueIn =
      (item.start - firstWordStart) * 1000 - (Date.now() - streamStartedAt);
...
router.post("/preview", async (req, res) => {
  try {
    ...
    return res.status(500).json({
      error: "Failed to generate preview captions",
    ...
    ws.on("message", async (message) => {
      try {
        ...
        if (!resolvedVideoPath) {
          ws.send(
            JSON.stringify({
              error: "Input video file not found for realtime captions",
            })
          );
          return;
        }
        ...
        if (ws.readyState === 1) {
          ws.send(
            JSON.stringify({
              error: error.message || "Realtime caption stream failed",
            })
          );
        }
      }
